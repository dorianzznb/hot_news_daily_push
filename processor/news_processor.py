...
def cleanup_processing_data(*data_items):
    """
    清理处理过程中的大数据项
    """
    for item_name, item_data in data_items:
        if item_data is not None:
            try:
                if hasattr(item_data, 'clear') and callable(item_data.clear):
                    item_data.clear()
                del item_data
                logger.debug(f"已清理处理数据: {item_name}")
            except Exception as e:
                logger.debug(f"清理处理数据 {item_name} 时出错: {str(e)}")
...
async def process_hotspot_with_summary(hotspots, hunyuan_api_key, max_workers=5, tech_only=False, use_cache=True):
    ...
    try:
            ...
            try:
                ...
                if needs_fetching:
                    ...
                    try:
                        ...
                        # 清理获取的内容变量
                        if fetched_content and fetched_content != content:
                            cleanup_processing_data(("fetched_content", fetched_content))
                    ...
                    if has_content: 
                        ...
                        try:
                            ...
                            try:
                                ...
                                # 清理BeautifulSoup对象
                                if soup:
                                    cleanup_processing_data(("soup", soup))
                    else:
                ...
                logger.error(f"处理单个项目时出错: {item_err}, 标题: {title}")
                # 返回原始项目，标记为处理失败
                return {
                    **item,
                    "is_processed": False,
                ...
                # 清理单个项目处理中的临时变量
                cleanup_processing_data(
                    ("html_content", html_content),
                    ("summary_result_ai", summary_result_ai)
                )
        ...
        try:
            ...
            tasks = [
                loop.run_in_executor(
                    executor,
                    lambda i=item: asyncio.run(process_single_item(i))
                )
            ...
            for i, completed_task in enumerate(completed_tasks):
                try:
                    if isinstance(completed_task, Exception):
                        logger.error(f"任务 {i} 执行失败: {completed_task}")
                        # 添加原始项目，标记为失败
        ...
        logger.error(f"处理热点数据时发生错误: {str(e)}")
        raise
    finally:
        # 最终清理和垃圾回收
        if executor:
